...
public:
	...
	int drop[7][7] = { 0 };
public:
	void CreateGrid(Grid& grd, Graphics& gfx, int move[7][7], int& step, bool& triggerscan, int& dist, int& score, bool& scoretrigger);
	...
	void Scan(Location& loc, Grid& grd, bool trigger, int& dist, int& score, bool& scoretrigger);
	void Drop(Location& from, Location& to, Grid& grd, Graphics& gfx, int& dist, int& drop);
private:
	static constexpr int xpos = 30;
	static constexpr int ypos = 30;
